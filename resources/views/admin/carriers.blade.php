   ...
   <div class="page-content">
      <div class="container-fluid">
         ...
         <div class="card custom-shadow rounded-lg border">
            ...
            <div class="">
               <form action="{{ route('carriers') }}" class="custom-validation" method="post" id="formValidatedOtherAddress">
                  @csrf
                           ...
                           <div class="searchwithdrop row bg-light py-3 px-2">
                                 <div class="col-md-4 form-group">
                                    <select class="form-control form-select select2" name="carrier_type">
                                       <option value="all" {{ 'all' == @$carrier_type ? 'selected' : '' }}>All Carrier</option>
                                       <option value="1" {{ '1' == @$carrier_type ? 'selected' : '' }}>Air Carrier</option>
                                       <option value="2" {{ '2' == @$carrier_type ? 'selected' : '' }}>Ocean Carrier</option>
                                       <option value="3" {{ '3' == @$carrier_type ? 'selected' : '' }}>Land Carrier</option>
                                    </select>
                                 </div>
               ...
               <table id="dataTableNew" class="table dt-responsive nowrap w-100">
                  ...
                  <tbody>
                     @if(!empty($list))
                        ...
                        @php
                            $carriers_types = DB::table('carriers_types')->where('id', $row->carrier_type)->select('name')->first();
                           ...
                           <tr>
                              ...
                              {{-- <td>
                                 <div class="square-switch">
                                    <input type="checkbox" id="square-switch{{ @$row->id }}" value="{{ @$row->status }}" switch="info" {{ (@$row->status == 1)? 'checked="checked"' : '' }} onchange="changePortStatus({{ @$row->id }}, $(this))" />
                              ...
                              <td>
                                 <a href="{{ route('viewCarrier', ['id' => $row->id]) }}" class="btn btn-sm btn-warning"><i class="far fa-eye"></i></a>
                                 <a href="{{ route('editCarrier', ['id' => $row->id]) }}" class="btn btn-sm btn-primary"><i class="far fa-edit"></i></a>
                                 <button data-toggle="tooltip" title="Delete Carrier" class="btn btn-sm btn-danger" onclick="deleteCarrier({{ @$row->id }})"><i class="fas fa-trash-alt"></i></button>
                              </td>
                           </tr>
                        @endforeach
                     @endif
